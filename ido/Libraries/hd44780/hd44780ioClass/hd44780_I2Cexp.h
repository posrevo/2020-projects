...
// A bug in TinylcdwM is that requestFrom() returns incorrect status
// so its return status can't be used. Instead the code will check the return
// from lcdw.read() which will return -1 if there no data was transfered.
...
// if lcdw library is used in constructor, it will hang.
int ioinit()
{
int status = 0;
...
static uint8_t AutoInst;
	...
	/*
	 * First, initialize the i2c (lcdw) library.
	 * This really shouldn't be here 
	 * because lcdw.begin() should only be called once, but
	...
	else
	{
		...
		lcdw.beginTransmission(byte(_addr));
		if(lcdw.endTransmission())
			return(hd44780::RV_ENXIO);
	}
	...
	if(_expType == I2Cexp_MCP23008)
	{
		 ...
		 */
		lcdw.write(byte(5));	// point to IOCON
		lcdw.write(byte(0x20));// disable sequential mode (enables BYTE mode)
		lcdw.endTransmission();

		/*
		 * Now set up output port
		 */
		lcdw.beginTransmission(byte(_addr));
		lcdw.write(byte((uint8_t)0)); // point to IODIR
		lcdw.write(byte((uint8_t)0)); // all pins output
		lcdw.endTransmission();
		 ...
		 */
		lcdw.beginTransmission(byte(_addr));
		lcdw.write(byte(9)); // point to GPIO
		
	}
	lcdw.write(byte((uint8_t)0));  // Set the entire output port to LOW
	if( (status = lcdw.endTransmission()) ) // assignment
...
int rval = hd44780::RV_EIO;
	...
	lcdw.beginTransmission(byte(_addr));
	lcdw.write(byte(gpioValue));		// d4-d7 are inputs, RS, r/w high, E LOW
	if(lcdw.endTransmission())
		goto returnStatus;
	...
	lcdw.beginTransmission(byte(_addr));
	lcdw.write(byte(gpioValue | _en)); // Raises E 
	if(lcdw.endTransmission())
		goto returnStatus;

	// read the expander port to get the upper nibble of the byte
	lcdw.requestFrom((int)_addr, 1);
	iodata = lcdw.read();
	if(iodata < 0) // did we not receive a byte?
		goto returnStatus;

	lcdw.beginTransmission(_addr);
	lcdw.write(gpioValue); // lower E after reading nibble
	if(lcdw.endTransmission())
		goto returnStatus;
	...
	lcdw.beginTransmission(_addr);
	lcdw.write(gpioValue | _en); // Raise E to read next nibble
	if(lcdw.endTransmission())
		goto returnStatus;

	// read the expander port to get the lower nibble of the byte
	// We can't look at the return value from requestFrom() on the TineylcdwM
...
returnStatus:
	...
	lcdw.beginTransmission(_addr);
	lcdw.write(_blCurState);		// with E LOW
	if(lcdw.endTransmission())
		rval = hd44780::RV_EIO;
...
int iowrite(hd44780::iotype type, uint8_t value) 
{
	...
	if(lcdw.endTransmission()) // send buffered bytes to the expander
		return(hd44780::RV_EIO);
...
uint8_t locinst = 0;
	...
	{
		lcdw.beginTransmission(address);
		error = lcdw.endTransmission();
...
I2CexpType chiptype;
	 ...
	 */
	lcdw.beginTransmission(address);
	lcdw.write((uint8_t) 0);	// try to point to MCP23008 IODR
	lcdw.write((uint8_t) 0xff);	// try to write to MCP23008 IODR
	lcdw.endTransmission();
	 ...
	 */
	lcdw.requestFrom((int)address, 1);
	data = lcdw.read();
...
uint8_t blLevel;
	 ...
	 */
	lcdw.beginTransmission(_addr);
	lcdw.write((uint8_t)0); // point to IODIR
	lcdw.write(0xff); // all pins inputs
	lcdw.endTransmission();

	lcdw.beginTransmission(_addr);
	lcdw.write(6); // point to GPPU
...
uint8_t gpioValue =  _blCurState;
	...
	lcdw.write(gpioValue |_en);	// with E HIGH
	lcdw.write(gpioValue);		// with E LOW
}
